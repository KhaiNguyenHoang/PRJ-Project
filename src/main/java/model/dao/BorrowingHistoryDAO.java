...
public class BorrowingHistoryDAO extends LibraryContext {
    ...
    // Ghi lại lịch sử trả sách
    public boolean addReturnHistory(Borrowing borrowing) {
        String insertReturnQuery = "INSERT INTO BorrowingHistory (MemberID, BookID, BookCopyId, BorrowDate, ReturnDate) " +
                "VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(insertReturnQuery)) {
            ps.setInt(1, borrowing.getMemberId());
            ps.setInt(2, borrowing.getBookId());
            ps.setInt(3, borrowing.getBookCopyId());
            ps.setTimestamp(4, new Timestamp(borrowing.getBorrowDate().getTime()));
            ps.setTimestamp(5, new Timestamp(borrowing.getReturnDate().getTime()));
